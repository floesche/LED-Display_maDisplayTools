...
| Date | Change |
|------|--------|
| 2026-04-01 | **Experiment Designer v0.6 — V2 Protocol + Plugins + 3 Editor Views** — Full YAML v2 migration: shared modules (`protocol-yaml.js`, `plugin-registry.js`), inline comment stripping in parser, v2 export/import with rig path + plugins (LEDControllerPlugin, BiasPlugin). New data model: conditions/phases use `commands[]` arrays. Three editor tabs: Visual (command card builder), Table (collapsible spreadsheet), Timeline (SVG multi-lane). Test suite: 130/130 JS, 35/35 MATLAB. CI workflow updated for v2. Browser export → MATLAB ProtocolParser roundtrip validated. |
